...
function isSupportedSourceFile(fileName: string): boolean {
  ...
  return lowerName.endsWith('.mxl') || lowerName.endsWith('.xml') || lowerName.endsWith('.musicxml')
}
...
function findMatchingBeatIndex(
  ...
  for (let index = startIndex; index < beats.length; index += 1) {
    ...
    if (requireSameVoice && beat.voice !== voice) {
      continue
    }
    if (!samePitchClassSet(beat.pitchClasses, expectedPitchClasses)) {
      continue
    }
    return index
  }
...
function mapPracticeTargetsToScoreGroups(
  score: AlphaScoreLike | null,
  targets: PracticeTarget[],
): string[] {
  const beats = collectScoreBeatDescriptors(score)
  if (!beats.length || !targets.length) {
    return []
  }
  ...
  for (const target of targets) {
    ...
    let matchIndex = findMatchingBeatIndex(
      beats,
      cursor,
      expectedPitchClasses,
      firstEvent.measure_number,
      firstEvent.voice,
      true,
    )

    if (matchIndex < 0) {
      matchIndex = findMatchingBeatIndex(
        beats,
        cursor,
        expectedPitchClasses,
        firstEvent.measure_number,
...
function selectTopPitchClasses(
  frames: WeightedPitchClass[][],
  size: number,
): number[] {
  if (!frames.length || size <= 0) {
    return []
  }
...
function toChromaWeightVector(weightedPitchClasses: WeightedPitchClass[]): number[] {
  ...
  if (!weightedPitchClasses.length) {
    return vector
  }
...
export function PartituraViewerV2Page() {
  const params = useParams()
  const [partitura, setPartitura] = useState<PartituraDetail | null>(null)
  const [status, setStatus] = useState('Carregando visualizacao v2...')
  ...
  const detectionCalibration = useMemo(
    () => DETECTION_CALIBRATIONS[detectionProfileId],
    [detectionProfileId],
  )
  ...
  useEffect(() => {
    detectionCalibrationRef.current = detectionCalibration
  ...
  const clearPracticeHighlight = useCallback(() => {
    ...
    highlighted.forEach((element) => {
      element.classList.remove('at-practice-target')
    })
  }, [])
  ...
  async function startPracticeMode() {
    if (!partitura || isPracticeActive || mediaStreamRef.current) {
      return
    }

    if (!practiceTargets.length) {
      setStatus('Nao ha notas detectadas nessa partitura para iniciar a pratica. Reimporte como MusicXML com notas em pauta.')
      return
    }
    ...
    try {
      ...
      setStatus('Pratica iniciada: toque a nota ou acorde alvo')
      syncPracticeHighlight(0)

      const [num] = (partitura.time_signature || '4/4').split('/')
      ...
      const detect = () => {
        ...
        if (chordWeightedWindowRef.current.length > CHORD_WINDOW_FRAMES) {
          chordWeightedWindowRef.current.shift()
        }
        ...
        const topCountForUi = Math.max(1, currentTarget?.expectedPitchClasses.length ?? 3)
        ...
        if (isPracticeActiveRef.current && !advanceLockRef.current) {
          ...
          if (target) {
            ...
            const combinedPitchClasses = selectTopPitchClasses(
              chordWeightedWindowRef.current,
              expectedPitchClassCount,
            )
            ...
            const chordEvaluation = matchPracticeTarget(
              ...
              calibration.chordMatchThreshold,
              false,
            )
            ...
            const isSingleNoteReady = Boolean(
              target.kind === 'note'
              && detection.pitch
              && Math.abs(detection.pitch.cents) <= calibration.maxCentsTolerance
              && stableFramesRef.current >= calibration.requiredStableFrames
            ...
            const isChordReady = Boolean(
              target.kind === 'chord'
              && chordEvaluation.isMatch
              && chordStableFramesRef.current >= calibration.chordStableFrames,
            )
            ...
            if (!isSingleNoteReady && !isChordReady) {
              ...
              if (isLastTarget) {
                ...
                setStatus(
                  target.kind === 'chord'
                    ? 'Acorde correto! Proximo alvo.'
                    : 'Nota correta! Proximo alvo.',
                )
                stableFramesRef.current = 0
                stableTokenRef.current = ''
                chordStableFramesRef.current = 0
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!id) {
        setStatus('Partitura invalida')
        return
      }

      try {
        const detail = await getPartitura(id)
        if (disposed) {
          return
        }
        ...
        if (!detail.events.length) {
          setStatus('Esta partitura foi importada sem notas detectadas. A visualizacao pode abrir, mas a pratica ficara indisponivel.')
        }

        if (!isSupportedSourceFile(detail.source_filename)) {
          setStatus('A visualizacao v2 com AlphaTab suporta apenas arquivos .xml/.mxl importados.')
          return
        }

        setStatus('Carregando arquivo fonte da partitura...')
        const sourceBlob = await getPartituraSourceFile(id)
        ...
        const api = new alphaTab.AlphaTabApi(containerRef.current, {
          core: {
            // Vite 8 + alphatab sem plugin: render no main thread evita travamentos silenciosos do worker.
            useWorkers: false,
            // Usa os arquivos SMuFL servidos em /public/font.
            fontDirectory: '/font/',
          },
          player: {
            enablePlayer: false,
        ...
        api.error.on((error) => {
          ...
          setStatus(`Falha ao renderizar partitura no AlphaTab: ${message}`)
        })
        ...
        api.renderFinished.on(() => {
          ...
          if (isPracticeActiveRef.current) {
            syncPracticeHighlight(currentPracticeIndexRef.current)
          }
          setStatus('')
        })

        api.scoreLoaded.on((score) => {
          practiceTargetGroupIdsRef.current = mapPracticeTargetsToScoreGroups(
            score as unknown as AlphaScoreLike,
        ...
        apiRef.current = api
        setStatus('Renderizando partitura v2...')
        const loadStarted = api.load(sourceBytes)
        if (!loadStarted) {
          throw new Error('AlphaTab recusou os dados do arquivo (formato invalido ou nao suportado).')
        }

        // Safety net: evita estado de carregamento infinito caso não haja callback.
        renderTimeout = setTimeout(() => {
          if (!disposed) {
            setStatus('A renderizacao da partitura demorou mais que o esperado. Tente recarregar a pagina ou reimportar o arquivo.')
          }
        }, 15000)
      } catch (error) {
        if (renderTimeout) {
        ...
        if (!disposed) {
          const message = error instanceof Error ? error.message : String(error)
          setStatus(`Falha ao carregar visualizacao v2 da partitura: ${message}`)
        }
      }
    }
    ...
    <main className="mx-auto min-h-screen w-full max-w-6xl px-4 py-8">
      <section className="card p-6">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl">{partitura?.title ?? 'Partitura (V2 - AlphaTab)'}</h1>
            {partitura && (
              <p className="text-sm text-slate-600">
                Fonte: {partitura.source_filename} | {partitura.time_signature} | {partitura.tempo_bpm} BPM
              </p>
            )}
          </div>
